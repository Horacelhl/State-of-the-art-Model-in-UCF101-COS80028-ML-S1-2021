...
# USE THIS
images = sorted(glob("UCF_10/*.jpeg"))
...
def plot_history(history):
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig(sec_path + 'MLP_training_history_UCF_10_v%d.png'%VERSION)
    plt.show()    
...
with strategy.scope():    
    ...
    model_checkpoint_callback = ModelCheckpoint(sec_path + 'weight_UCF_10_v%d.hdf5'%VERSION, save_best_only=True, monitor='val_loss', mode='min')
...
#saving model
    ...
    with open(sec_path + 'MLP_history_UCF_10_v%d.csv'%VERSION, mode='w') as f:
        result.to_csv(f)
        hist_df.to_csv(f)
